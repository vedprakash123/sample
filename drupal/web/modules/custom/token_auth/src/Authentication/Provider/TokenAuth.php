...
class TokenAuth extends Cookie implements AuthenticationProviderInterface {
  ...
  public function applies(Request $request) {
    ...
    //$token = \Drupal::request()->header->all();
    //$token = $token->getHeaders();
    \Drupal::logger('token_auth')->info('Request Token Header AU l: ' . print_r($token, TRUE));
  ...
  protected function isCorrectToken($tok) {
    ...
    \Drupal::logger('token_auth')->info('Node Type: ' . print_r($tok, TRUE));
